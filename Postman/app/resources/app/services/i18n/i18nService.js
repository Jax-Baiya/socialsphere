...
function _refreshMenu () {
  try {
    ...
    appSettings.get('shortcutsDisabled', (error, isShortcutsDisabled) => {
      if (error) {
        pm.logger.error('i18nService~_refreshMenu : Error while retrieving shortcutsDisabled flag', error);
        return;
      }
    ...
    pm.logger.error('i18nService~_refreshMenu : Error while refreshing OS menu', error);
  }
}
...
class i18nService {
  ...
  _setLocaleCache (locale) {
    ...
    appSettings.set(STORAGE_KEY, locale, (error) => {
      error && pm.logger.error('i18nService~_setLocaleCache : Error while setting the user preferred locale', error);
    });
  }
  ...
  init = () => {
    ...
    appSettings.get(STORAGE_KEY, (error, locale) => {
      ...
      if (error) {
        ...
        pm.logger.error('i18nService~init : Error while retrieving stored locale preference', error);
      } else {
      ...
      try {
        ...
        i18n
          .init({
            ...
            missingKeyHandler: (lngs, ns, key) => {
              pm.logger.error('i18n ~ missingKeyHandler', lngs, ns, key);
            }
          });
        ...
        pm.eventBus.channel(i18n_EVENT_BUS).subscribe((event = {}) => {
          ...
          if (name === LOCALE_CHANGED_EVENT && [i18n_EVENT_NAMESPACE, i18n_MAIN_EVENT_NAMESPACE].includes(namespace)) {
            if (!data || !data.locale) {
              pm.logger.error('i18nService~localeChanged event bus - locale is not supplied');
              return;
            }
        ...
        pm.logger.error('i18nService~init : Failed to initialize i18n', err);
      }
    });
  }
}
