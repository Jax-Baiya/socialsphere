...
async function getPlatformSpecs () {
  try {
    ...
    const memoryInformation = memory.getMemoryInformation();
...
async function getUsageStats () {
  const { free: freeMemory, total: totalMemory } = memory.getMemoryInformation();

  return {
    system: {
      cpu: await cpu.getCPUUsagePromise(50),
      memory: freeMemory * 100 / totalMemory,
      totalMemoryBytes: totalMemory * memory.BYTES_TO_MB_MULTIPLIER,
    }
  };
}
