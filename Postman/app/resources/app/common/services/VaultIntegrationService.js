...
class VaultInterface extends events.EventEmitter {
  ...
  */
   ...
   * @param {string | HashicorpSecretKey | AWSSecretKey} _secretId
...
class HashicorpVault extends VaultInterface {
  ...
  */
  ...
  async init (caCertPath) {
    ...
    if (caPath) {
      ...
      if (platform === 'win32') {
        caPath = require('path').normalize(caCertPath.slice(1));
      }
  ...
  */
  async performLoginFlow (urlCallback) {
    ...
    try {
      ...
      this.#tokenResult = await this.loginUsingJWT(jwt);
      this._broadcastUserInfo();
   ...
   */
  isSupportedSecret (secret) {
    if (new AzureVault().isSupportedSecret(secret)) {
      return true;
    }

    if (!_.isObject(secret) || _.get(secret, 'provider.id') != HashicorpVault.PROVIDER_ID) {
      return false;
    }
  ...
  */
  _parseSecret (secretId) {
    const HASHICORP_DELIMITER = ':@:@:';
    if ('secretId' in secretId) {
      return {
        ...secretId,

        secretEngine: secretId.secretId.split(HASHICORP_DELIMITER)[0],
        secretPath: secretId.secretId.split(HASHICORP_DELIMITER)[1],
        secretKey: secretId.secretId.split(HASHICORP_DELIMITER)[2],
      };
    } else {
      return secretId;
    }
 ...
 */
 ...
 static LOGGED_IN_DURATION = 60 * 60 * 2; // 2 hours
...
class VaultManager {
  static types = [AzureVault, HashicorpVault, SecretsManagerVault];
  ...
  */
  ...
  isSupportedSecret (secretId) {
    for (const vault of Object.values(this.initialized)) {
  ...
  async resolveSecrets (secrets, abortSignal) {
    ...
    secrets.forEach((secret, idx) => {
      ...
      for (const [vaultId, vault] of Object.entries(this.initialized)) {
        if (vault.isSupportedSecret(secret.secretId)) {
