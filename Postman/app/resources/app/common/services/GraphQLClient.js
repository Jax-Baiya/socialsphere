...
async function executeSubsription (
  ...
  const webSocketImpl = createCustomWebSocket(
    { headers, followRedirects, rejectUnauthorized: verifySSLCertificates },
    websocketEvents
  );
  let asyncIterable;
  let abort;
...
async function getAuthHeaders (url, auth, graphQLParams) {
  ...
  return new Promise((resolve, reject) => {
    authorizeRequest(new collectionSDK.Request({ url: new collectionSDK.Url(url), auth, body: JSON.stringify(graphQLParams) }), (err, result) => {
      if (err) {
        reject(err);
      } else {
        resolve(
